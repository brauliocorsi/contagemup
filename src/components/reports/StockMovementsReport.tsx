...
import { Calendar as CalendarIcon, FileDown, TrendingUp, TrendingDown, Search, Filter, X } from 'lucide-react';
...
export function StockMovementsReport() {
  ...
  const [sortColumn, setSortColumn] = useState<'date' | 'product' | 'quantity' | null>('date');
  ...
  const getSortIcon = (column: 'date' | 'product' | 'quantity') => {
    if (sortColumn !== column) {
  ...
  const filteredMovements = useMemo(() => {
    ...
    return result;
  }, [movements, filterType, searchTerm, dateRange, sortColumn, sortDirection]);
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Data', 'Hora', 'Tipo', 'Código', 'Produto', 'Quantidade', 'Motivo', 'Referência'];
